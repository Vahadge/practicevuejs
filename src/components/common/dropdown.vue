...
<template>
  ...
  <div class="relative z-0 w-full mb-5 mt-2 group">
    ...
    class="mt-2 block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2
     ...
     focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="Select">
    <option value="">Select</option>
    <option v-for="option in props.options"  :id="option.value" :key="option.value">{{ option.text }}</option>
  </select>
  <label
    :for="props.id"
